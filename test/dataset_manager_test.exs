defmodule DatasetManagerTest do
  ...
  describe "load/2" do
    test "loads MMLU STEM dataset" do
      {:ok, dataset} = CrucibleDatasets.load(:mmlu_stem, TestHelper.data_opts(sample_size: 50))
    ...
    test "loads HumanEval dataset" do
      {:ok, dataset} = CrucibleDatasets.load(:humaneval, TestHelper.data_opts(sample_size: 10))
    ...
    test "loads GSM8K dataset" do
      {:ok, dataset} = CrucibleDatasets.load(:gsm8k, TestHelper.data_opts(sample_size: 20))
    ...
    test "caches loaded datasets" do
      {:ok, dataset1} = CrucibleDatasets.load(:mmlu_stem, TestHelper.data_opts(sample_size: 10))
      {:ok, dataset2} = CrucibleDatasets.load(:mmlu_stem, TestHelper.data_opts(sample_size: 10))
    ...
    test "respects cache: false option" do
      {:ok, dataset} =
        CrucibleDatasets.load(:mmlu_stem, TestHelper.data_opts(sample_size: 10, cache: false))
  ...
  describe "evaluate/2" do
    ...
    test "evaluates predictions with multiple metrics" do
      {:ok, dataset} = CrucibleDatasets.load(:gsm8k, TestHelper.data_opts(sample_size: 3))
  ...
  describe "cache management" do
    test "lists cached datasets" do
      {:ok, _dataset} = CrucibleDatasets.load(:mmlu_stem, TestHelper.data_opts(sample_size: 10))
    ...
    test "clears all cache" do
      {:ok, _dataset1} = CrucibleDatasets.load(:mmlu_stem, TestHelper.data_opts(sample_size: 10))
      {:ok, _dataset2} = CrucibleDatasets.load(:gsm8k, TestHelper.data_opts(sample_size: 10))
