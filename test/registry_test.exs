defmodule CrucibleDatasets.RegistryTest do
  ...
  test "lists available datasets" do
    assert Enum.sort(Registry.list_available()) == [:gsm8k, :humaneval, :mmlu, :mmlu_stem]
  end
  ...
  describe "filters" do
    ...
    test "list_by_difficulty/1" do
      ...
      assert Registry.list_by_difficulty("medium") == [:gsm8k, :humaneval]
    end
  ...
  test "stats includes aggregate counts" do
    ...
    assert Enum.sort(stats.domains) == ["code", "general_knowledge", "math", "stem"]
  ...
  test "summary renders readable output" do
    ...
    assert summary =~ "Total Datasets: 4"
