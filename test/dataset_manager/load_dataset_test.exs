defmodule CrucibleDatasets.LoadDatasetTest do
  use ExUnit.Case, async: false

  import Plug.Conn

  alias CrucibleDatasets.{Dataset, DatasetDict, IterableDataset}
  ...
  test "load_dataset returns DatasetDict when split is nil", %{repo_id: repo_id} do
    {:ok, dataset_dict} = CrucibleDatasets.load_dataset(repo_id)
  ...
  test "load_dataset returns Dataset when split is specified", %{repo_id: repo_id} do
    {:ok, dataset} = CrucibleDatasets.load_dataset(repo_id, split: "train")
  ...
  test "load_dataset streaming returns IterableDataset", %{repo_id: repo_id} do
    {:ok, iterable} = CrucibleDatasets.load_dataset(repo_id, split: "train", streaming: true)
